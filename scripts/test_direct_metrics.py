...
def test_direct_metrics_edm(model, input_shape, lr_input_shape, n_repeat=100, use_float16=False, jit_compile=False):
    cudnn.benchmark = True
    ...
    x = torch.FloatTensor(*input_shape).uniform_(0., 1.)
    x = x.cuda()
    t = torch.FloatTensor(input_shape[0]).uniform_(0., 1.)
    t = t.cuda()
    lr = torch.FloatTensor(*lr_input_shape).uniform_(0., 1.)
    lr = lr.cuda()
    print(f'Input shape: {x.shape}')
    model = model.cuda()
    model.eval()
    ...
    with context():
        with torch.inference_mode():
            print('warmup ...')
            for _ in tqdm.tqdm(range(n_repeat)):  
                model(x, t, lr)  # Make sure CUDNN to find proper algorithms, especially for convolutions.
                torch.cuda.synchronize()
